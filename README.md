...
```shell
curl -L https://github.com/fortio/fortio/releases/download/v1.11.0/fortio-linux_x64-1.11.0.tgz \
 | sudo tar -C / -xvzpf -
# or the debian package
wget https://github.com/fortio/fortio/releases/download/v1.11.0/fortio_1.11.0-1_amd64.deb
dpkg -i fortio_1.11.0-1_amd64.deb
# or the rpm
rpm -i https://github.com/fortio/fortio/releases/download/v1.11.0/fortio-1.11.0-1.x86_64.rpm
```
...
On Windows, download https://github.com/fortio/fortio/releases/download/v1.11.0/fortio_win_1.11.0.zip and extract all to some location then using the Windows Command Prompt:
```
cd fortio
fortio.exe server
```
...
Fortio 1.11.0 tcp-echo server listening on [::]:8078
Fortio 1.11.0 grpc 'ping' server listening on [::]:8079
Fortio 1.11.0 https redirector server listening on [::]:8081
Fortio 1.11.0 echo server listening on [::]:8080
Data directory is /Users/ldemailly/go/src/fortio.org/fortio
UI started - visit:
http://localhost:8080/fortio/
(or any host/ip reachable on this server)
14:11:05 I fortio_main.go:233> All fortio 1.11.0 release go1.15.2 servers started!
```
...
Fortio 1.11.0 grpc ping server listening on port :8079
Fortio 1.11.0 echo server listening on port 10.10.10.10:8088
```
...
Fortio 1.11.0 grpc 'ping' server listening on [::]:8079
Fortio 1.11.0 https redirector server listening on [::]:8081
Fortio 1.11.0 echo server listening on /tmp/fortio-uds-http
UI started - visit:
fortio curl -unix-socket=/tmp/fortio-uds-http http://localhost/fortio/
14:58:45 I fortio_main.go:217> All fortio 1.11.0 unknown go1.10.3 servers started!
...
Φορτίο version 1.11.0 unknown go1.10.3 echo debug server up for 2m3.4s on ldemailly-macbookpro - request from
...
Host: foo.bar
User-Agent: fortio.org/fortio-1.11.0
...
$ fortio echo-server &
Fortio 1.11.0 tcp-echo server listening on [::]:8078
19:45:30 I fortio_main.go:238> All fortio 1.11.0 release go1.15.2 servers started!
$ load  -qps -1 -n 100000 tcp://localhost:8078
Fortio 1.11.0 running at -1 queries per second, 16->16 procs, for 100000 calls: tcp://localhost:8078
...
Https redirector running on :8081
Fortio 1.11.0 grpc ping server listening on port :8079
Fortio 1.11.0 echo server listening on port localhost:8080
...
Fortio 1.11.0 running at 8 queries per second, 8->8 procs, for 5s: http://www.google.com
...
Fortio 1.11.0 running at 8 queries per second, 8->8 procs, for 5s: https://fortio-stage.istio.io
...
Φορτίο version 1.11.0 echo debug server up for 39s on ldemailly-macbookpro - request from [::1]:65055
...
[...]
Fortio 1.11.0 Multi on 5554 server listening on [::]:5554
...
Φορτίο version 1.11.0 unknown go1.15.2 echo debug server up for 1m9.3s on C02C77BHMD6R - request from [::1]:51020
...
http://localhost:8080/fortio/
(or any host/ip reachable on this server)
Fortio 1.11.0 proxy for [::1]:8080 server listening on [::]:8888
Fortio 1.11.0 proxy for [::1]:8080 server listening on [::1]:8889
```
