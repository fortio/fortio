...
var (
	...
	BufferSizeKb = 128
	// MaxPayloadSizeKb is the maximum size of payload to be generated by the EchoHandler size= argument. In kilobytes defaults to 256k.
...
func generateSize(sizeInput string) (size int) {
	...
	if len(lst) == 1 && !strings.ContainsRune(sizeInput, ':') {
		...
		if size > MaxPayloadSizeKb*1024 {
			log.Warnf("Requested size %d greater than max size %d, using max instead", size, MaxPayloadSizeKb*1024)
			size = MaxPayloadSizeKb*1024
		}
		return size
	}
	weights := make([]float32, len(lst))
	...
	for _, entry := range lst {
		...
		if s > MaxPayloadSizeKb*1024 {
			log.Warnf("Requested size %d greater than max size %d, using max instead", s, MaxPayloadSizeKb*1024)
			s = MaxPayloadSizeKb*1024
		}
		percStr := removeTrailingPercent(l2[1])
		p, err := strconv.ParseFloat(percStr, 32)
		if err != nil || p < 0 || p > 100 {
...
var payload = make([]byte, MaxPayloadSizeKb*1024)
