...
var (
	...
	BufferSizeKb = 128
	// MaxPayloadSizeKb is the maximum size of payload to be generated by echo handler
...
func NewFastClient(o *HTTPOptions) Fetcher {
	...
	bc.buffer = make([]byte, changeKbToBytes(BufferSizeKb))
	if bc.port == "" {
...
// Change kb to bytes
func changeKbToBytes(kb int) int {
	return kb * 1024
}
...
func generateSize(sizeInput string) (size int) {
	...
	if len(lst) == 1 && !strings.ContainsRune(sizeInput, ':') {
		...
		if size > changeKbToBytes(MaxPayloadSizeKb) {
			log.Warnf("Requested size %d greater than max size %d, using max instead", size, changeKbToBytes(MaxPayloadSizeKb))
			size = changeKbToBytes(MaxPayloadSizeKb)
		}
		return size
	}
	weights := make([]float32, len(lst))
	...
	for _, entry := range lst {
		...
		if s > changeKbToBytes(MaxPayloadSizeKb) {
			log.Warnf("Requested size %d greater than max size %d, using max instead", s, changeKbToBytes(MaxPayloadSizeKb))
			s = changeKbToBytes(MaxPayloadSizeKb)
		}
		percStr := removeTrailingPercent(l2[1])
		p, err := strconv.ParseFloat(percStr, 32)
		if err != nil || p < 0 || p > 100 {
...
var payload = make([]byte, changeKbToBytes(MaxPayloadSizeKb))
